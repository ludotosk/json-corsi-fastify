...
fastify.get('/tabella', function(request, reply) {
    ...
    if (!saved) {
        ...
        corsiFiltrati.forEach(corso => {
            tabella += `<tr class="even:bg-slate-50"><td class="text-red-600 border px-1"><a target="_blank" rel="noopener" href="${corso.h}">${corso.n}</a></td><td>${corso.s}</td><td>${corso.u}</td></tr>`
        })
